...
function sanitizedError(error: unknown): string {
  return error instanceof Error ? error.message.slice(0, 240) : "Unknown worker error";
}
...
async function enqueueRegistrationEmailAfterBib(registrationId: string): Promise<void> {
  await withTransaction(async (client) => {
    ...
    if (!encryptedRegistrationCode) {
      logger.warn("Registration email after BIB skipped because code is unavailable", {
        registrationId,
      });
      return;
    }
...
async function handleRegistrationEmail(job: BackgroundJob): Promise<void> {
  ...
  const registrationCode = decryptString(payloadString(job, "encryptedRegistrationCode"));
  ...
  try {
    ...
    await sendRegistrationConfirmationEmail({
      ...
      eventPeriod: formatDateRange(summary.event.activityStartsAt, summary.event.activityEndsAt),
    ...
    await withTransaction(async (client) => {
      await markEmailDeliverySent({ eventRegistrationId: registrationId, emailDeliveryId }, client);
      await updateRegistrationEmailStatus({ registrationId, emailStatus: "SENT" }, client);
    });
  } catch (error) {
    await withTransaction(async (client) => {
      ...
      await updateRegistrationEmailStatus({ registrationId, emailStatus: "FAILED" }, client);
    });
    throw error;
  }
}
...
async function handleSubmissionValidationEmail(job: BackgroundJob): Promise<void> {
...
async function pollOnce(): Promise<boolean> {
  const job = await withTransaction((client) => claimNextBackgroundJob(workerId, client));
