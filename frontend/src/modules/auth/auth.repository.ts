...
export async function createAdminSession(
  ...
  await query(
    `
      ...
      VALUES ($1, $2, $3, $4, $4)
    `,
    [input.adminUserId, input.sessionTokenHash, input.csrfTokenHash, input.expiresAt],
    client,
  );
}

export async function revokeAdminSession(sessionId: string, client?: PoolClient): Promise<void> {
  await query(
    `
...
export async function recordLoginAttempt(
  ...
  await query(
    ...
    `,
    [input.normalizedEmail, input.success, input.reason, input.ipAddress, input.userAgent],
    client,
  );
}
  ...
  const result = await query<AdminUserRow>(
    `
      ...
      RETURNING
        ...
        updated_at
    `,
    [input.normalizedEmail, input.displayEmail, input.fullName, input.passwordHash],
    client,
  );
