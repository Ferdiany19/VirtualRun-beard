...
import type { AdminSessionTokens, AuthenticatedAdmin } from "@/modules/auth/auth.types";
import {
...
import { hashAdminPassword, verifyAdminPassword } from "@/modules/auth/password";
  ...
  return withTransaction(async (client) => {
    ...
    if (recentFailedAttempts >= MAX_FAILED_ATTEMPTS) {
      ...
      throw new ApplicationError({
        ...
        safeMessage: "Terlalu banyak percobaan login. Coba lagi beberapa menit lagi.",
        statusCode: 429,
      });
    }
    ...
    const expiresAt = new Date(Date.now() + SESSION_DURATION_HOURS * 60 * 60_000);
