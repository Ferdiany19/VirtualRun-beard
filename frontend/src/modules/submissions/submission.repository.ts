...
function mapSubmission(row: SubmissionRow): SubmissionRecord | null {
  if (!row.submission_id || !row.submission_status || row.revision_count === null) {
    return null;
  }
...
export async function getParticipantSubmissionCategory(
  input: { eventRegistrationId: string; registrationCategoryId: string; eventSlug: string },
  client?: PoolClient,
...
export async function listSubmissionRevisions(
  submissionId: string,
  client?: PoolClient,
): Promise<Array<SubmissionRevisionRecord & { file: SubmissionFileRecord | null }>> {
