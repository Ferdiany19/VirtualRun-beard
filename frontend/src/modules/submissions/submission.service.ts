...
function parseDistanceMeter(input: string): number {
  ...
  const meters = Number.parseInt(whole, 10) * 1000 + Number.parseInt(fraction.padEnd(3, "0"), 10);
...
function normalizeActivityUrl(value: string | null): string | null {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) {
    throw new ApplicationError({
      ...
      safeMessage: "URL aktivitas harus HTTPS dan tidak boleh memuat credential.",
      statusCode: 400,
    });
  }
...
function detectImageMime(buffer: Buffer): "image/jpeg" | "image/png" | "image/webp" {
...
async function assertRateLimit(input: {
  ...
  const identifiers = [input.participantId, input.requestContext.ipAddress ?? "unknown-ip"];
...
function assertSubmissionAllowed(
  ...
  if (
    ...
    throw new ApplicationError({
      ...
      safeMessage: "Status hasil saat ini tidak dapat diperbaiki pada fase ini.",
      statusCode: 400,
    });
  }
}

function assertActivityDate(category: ParticipantSubmissionCategory, activityDate: string): void {
...
export async function submitParticipantRevision(input: {
  ...
  requestContext: RequestContext;
}): Promise<{ submissionId: string; revisionId: string; revisionNumber: number }> {
  ...
  return withTransaction(async (client) => {
    ...
    await markCurrentRevisionSuperseded(lockedSubmission.currentRevisionId, client);
    ...
    await recordSecurityAttempt(
      {
        ...
        identifierHash: rateLimitIdentifier(`SUBMIT_REVISION:${category.participant.id}`),
      ...
      await enqueueBackgroundJob(
        {
          ...
          payload: { submissionId: lockedSubmission.id, revisionId: revision.id },
          maximumAttempts: 5,
        },
        client,
      );
    ...
    if (isDatabaseErrorCode(error, "23505")) {
      throw new ApplicationError({
        ...
        safeMessage: "Submit bersamaan terdeteksi. Muat ulang halaman dan periksa hasil terakhir.",
        statusCode: 409,
        cause: error,
      });
    }
...
export async function cleanExpiredUploads(): Promise<number> {
  ...
  await markUploadSessionsExpired(expiredObjects.map((item) => item.uploadSessionId));
