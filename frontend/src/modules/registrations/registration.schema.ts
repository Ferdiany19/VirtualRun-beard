...
export const publicRegistrationSchema = z.object({
  ...
  cityOrRegency: z.string().trim().min(2, "Kota/kabupaten wajib diisi.").max(120),
  ...
  turnstileToken: z.string().trim().min(1, "Verifikasi keamanan wajib diselesaikan.").max(4096),
  idempotencyKey: z.string().uuid(),
});

export const participantAccessSchema = z.object({
  displayEmail: z.string().trim().email("Email belum valid.").max(180),
  registrationCode: z.string().trim().min(8, "Kode registrasi wajib diisi.").max(32),
});
...
export function parsePublicRegistrationFormData(formData: FormData): PublicRegistrationInput {
...
export function parseParticipantAccessFormData(formData: FormData): ParticipantAccessInput {
