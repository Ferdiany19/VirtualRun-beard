...
import type { SubmissionDetail, SubmissionStatus } from "@/modules/submissions/submission.types";
import {
...
async function assertCanViewEventValidation(
  ...
  throw forbidden("Admin cannot view event validation", "Anda tidak ditugaskan ke event ini.");
}
...
function notificationJobFor(action: ValidationAction): ValidationNotificationJob | null {
...
function buildWarnings(
  ...
  const distanceOutside = Math.abs(distanceDelta) > detail.category.distanceToleranceMeters;
  warnings.push({
    code: "DISTANCE_TOLERANCE",
    tone: distanceOutside ? "warning" : "neutral",
    label: distanceOutside ? "Jarak di luar toleransi" : "Jarak dalam toleransi",
  ...
  const paceSeconds = Math.round(revision.elapsedTimeSeconds / (revision.distanceMeter / 1000));
...
export async function listValidationQueueForAdmin(input: {
  ...
  return listValidationQueue({ eventIds, adminUserId: input.admin.id, filters: parsed });
}
...
export async function listEventValidationQueue(input: {
  ...
  const parsed = validationQueueSchema.parse({ ...input.filters, eventId: input.eventId });
...
function assertReviewVersion(
  ...
  if (locked.reviewVersion !== expectedReviewVersion) {
    throw new ApplicationError({
      ...
      safeMessage: "Submission berubah saat sedang direview. Muat ulang halaman.",
      statusCode: 409,
    });
  }
}

function assertCurrentRevision(locked: LockedSubmissionForValidation, revisionId: string): void {
  if (!locked.currentRevisionId || locked.currentRevisionId !== revisionId) {
    throw new ApplicationError({
      code: "CONFLICT",
      message: "Current revision changed",
      safeMessage: "Revisi terbaru sudah berubah. Muat ulang halaman sebelum menyimpan keputusan.",
      statusCode: 409,
    });
  }
}

function assertOwnsClaim(locked: LockedSubmissionForValidation, admin: AuthenticatedAdmin): void {
  if (
    ...
    throw new ApplicationError({
      ...
      safeMessage: "Claim review sudah tidak aktif. Ambil ulang claim sebelum menyimpan keputusan.",
      statusCode: 409,
    });
  }
}
...
export async function claimSubmissionForReview(input: {
  admin: AuthenticatedAdmin;
  form: { submissionId: string; expectedReviewVersion: number; reason?: string | null };
  ...
  await withTransaction(async (client) => {
    ...
    const action: ValidationAction = activeClaimByOther ? "OVERRIDE_REVIEW_CLAIM" : "START_REVIEW";
    const resultingStatus = assertStatusTransition({ action, previousStatus: locked.status });
    ...
    await createValidationReview(
      {
        ...
        resultingStatus,
        reasonCode: action === "OVERRIDE_REVIEW_CLAIM" ? "CLAIM_OVERRIDE" : null,
        internalNote: parsed.reason ?? null,
        metadata: {
          previousClaimedBy: locked.reviewClaimedByAdminUserId,
          previousClaimExpiresAt: locked.reviewClaimExpiresAt?.toISOString() ?? null,
        },
      },
      client,
    );
    await createAuditLog(
      {
        ...
        action: action === "OVERRIDE_REVIEW_CLAIM" ? "REVIEW_CLAIM_OVERRIDDEN" : "REVIEW_CLAIMED",
...
export async function saveValidationDecision(input: {
  ...
  await withTransaction(async (client) => {
    ...
    const isReopen = parsed.action === "REOPEN_SUBMISSION" || parsed.action === "RESTORE_TO_REVIEW";
    if (!isReopen) {
      ...
      await updateSubmissionDecision(
        {
          submissionId: locked.submissionId,
          status: resultingStatus,
          approvedRevisionId: resultingStatus === "APPROVED" ? parsed.revisionId : null,
          approvedByAdminUserId: resultingStatus === "APPROVED" ? input.admin.id : null,
          rankingEligible: resultingStatus === "APPROVED",
          rankingExclusionReason:
            resultingStatus === "APPROVED" ? null : (reasonCode ?? resultingStatus),
          participantVisibleNote: parsed.participantVisibleNote ?? null,
          reasonCode,
        },
        client,
    ...
    await createAuditLog(
      {
        ...
        eventId: input.eventId,
        newValues: { adminUserId: assignment.adminUserId, reason: parsed.reason },
        ipAddress: input.requestContext.ipAddress,
        userAgent: input.requestContext.userAgent,
        correlationId: input.requestContext.correlationId,
      },
...
export async function getValidationNotificationSummary(input: {
  ...
  return {
    ...
    participantVisibleNote:
      review?.participantVisibleNote ?? detail.submission.latestParticipantVisibleNote,
    contact:
