...
import { getPrivateObject, putPrivateObject } from "@/modules/storage/storage.service";
...
function sanitizedError(error: unknown): string {
  return error instanceof Error ? error.message.slice(0, 240) : "Unknown certificate error";
}
...
export async function enqueueCertificatesForCompletedEvent(
  ...
  const eligibleRows = await listCertificateEligibleRowsForEvent(eventId, client);
  let queuedCount = 0;
...
export async function renderCertificate(certificateId: string): Promise<void> {
  ...
  try {
    ...
    `);
    const output = await sharp(template).composite([{ input: overlay }]).png().toBuffer();
    ...
    await markCertificateFailed({ certificateId, sanitizedError: sanitizedError(error) });
    throw error;
  }
}

export async function sendCertificateEmailForCertificate(certificateId: string): Promise<void> {
