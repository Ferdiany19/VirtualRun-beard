...
import { getEventById, getEventByIdForAdmin } from "@/modules/events/event.repository";
import type { CategoryInput, EventCategoryRecord } from "@/modules/categories/category.types";
...
export async function createManagedCategory(input: {
  ...
  return withTransaction(async (client) => {
    ...
    try {
      const category = await createCategory(input.eventId, input.category, client);
...
export async function updateManagedCategory(input: {
  ...
  return withTransaction(async (client) => {
    ...
    try {
      const category = await updateCategory(input.categoryId, input.category, client);
      await createAuditLog(
        {
          actorType: "ADMIN_USER",
          actorId: input.admin.id,
...
export async function setManagedCategoryActiveStatus(input: {
  ...
  return withTransaction(async (client) => {
    ...
    const category = await setCategoryActiveStatus(input.categoryId, input.isActive, client);
    await createAuditLog(
      {
        actorType: "ADMIN_USER",
        actorId: input.admin.id,
