...
const envSchema = z
  .object({
    ...
    PARTICIPANT_SESSION_COOKIE_NAME: z.string().min(1).default("vrb_participant_session"),
    PARTICIPANT_CSRF_COOKIE_NAME: z.string().min(1).default("vrb_participant_csrf"),
    ...
    BIB_TEMPLATE_MAX_BYTES: z.coerce.number().int().positive().default(4_000_000),
    SUBMISSION_SCREENSHOT_MAX_BYTES: z.coerce.number().int().positive().default(6_000_000),
    REVIEW_CLAIM_DURATION_MINUTES: z.coerce.number().int().positive().default(30),
  ...
  .superRefine((value, context) => {
    if (value.NODE_ENV === "production" && value.TURNSTILE_DEVELOPMENT_BYPASS === "true") {
      context.addIssue({
