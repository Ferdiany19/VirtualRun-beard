...
export function assertStatusTransition(input: {
  ...
  const transitions: Partial<
    Record<
      ValidationAction,
      Partial<Record<SubmissionStatus, SubmissionStatus>>
    >
  > = {
    START_REVIEW: {
  ...
  if (!resultingStatus) {
    throw new ApplicationError({
      ...
      safeMessage:
        'Transisi status validation tidak valid untuk kondisi saat ini.',
      statusCode: 409,
    });
  }
...
export function assertReasonCodeForAction(input: {
  ...
  const reasonCode = input.reasonCode
    ? (input.reasonCode as ValidationReasonCode)
    : null;
  ...
  if (input.action === 'REQUEST_REVISION') {
    if (
      !reasonCode ||
      !revisionRequestReasonCodes.includes(
        reasonCode as RevisionRequestReasonCode,
      )
    ) {
      throw invalidReason();
    }
    ...
    if (
      reasonCode === 'OTHER_REVISION_REQUIRED' &&
  ...
  if (input.action === 'REJECT') {
    if (
      !reasonCode ||
      !rejectionReasonCodes.includes(reasonCode as RejectionReasonCode)
    ) {
      throw invalidReason();
    }
    if (!input.participantVisibleNote) {
  ...
  if (input.action === 'DISQUALIFY') {
    if (
      !reasonCode ||
      !disqualificationReasonCodes.includes(
        reasonCode as DisqualificationReasonCode,
      )
    ) {
      throw invalidReason();
    }
  ...
  if (
    input.action === 'REOPEN_SUBMISSION' ||
    input.action === 'RESTORE_TO_REVIEW'
  ) {
    if (!input.internalNote) {
      throw new ApplicationError({
        code: 'VALIDATION_FAILED',
