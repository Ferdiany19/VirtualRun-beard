...
function buildRegistrationEmail(input: RegistrationEmailInput): {
  subject: string;
  body: string;
} {
  return {
...
function buildSubmissionValidationEmail(
  ...
  return {
    ...
    body: [
      ...
      input.participantVisibleNote
        ? `Catatan: ${input.participantVisibleNote}`
        : null,
      '',
      `Akses peserta: ${input.participantAccessUrl}`,
      `Kontak organizer: ${input.contact}`,
...
function buildCertificateEmail(input: CertificateEmailInput): {
  subject: string;
  body: string;
} {
  return {
