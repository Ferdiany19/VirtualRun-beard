...
function assertCategoryRequirements(
  ...
  for (const category of categories) {
    if (
      (category.minimumAgeYears !== null ||
        category.maximumAgeYears !== null) &&
      !input.dateOfBirth
    ) {
      throw new ApplicationError({
        code: 'VALIDATION_FAILED',
    ...
    if (input.dateOfBirth) {
      ...
      if (
        ...
        throw new ApplicationError({
          ...
          safeMessage:
            'Usia peserta belum sesuai dengan kategori yang dipilih.',
          statusCode: 400,
        });
      }
    }
    ...
    if (
      category.genderDivision === 'MALE' ||
      category.genderDivision === 'FEMALE'
    ) {
      if (!input.gender || input.gender !== category.genderDivision) {
        throw new ApplicationError({
          ...
          safeMessage:
            'Gender peserta belum sesuai dengan kategori yang dipilih.',
          statusCode: 400,
        });
      }
    }
...
async function verifyTurnstile(
  ...
  if (
    env.TURNSTILE_DEVELOPMENT_BYPASS === 'true' &&
  ...
  if (!result.success) {
    throw new ApplicationError({
      ...
      safeMessage:
        'Verifikasi keamanan belum berhasil. Coba ulangi beberapa saat lagi.',
      statusCode: 403,
    });
  }
}
...
export async function registerParticipantForEvent(input: {
  ...
  const normalizedPhone = normalizeIndonesianPhone(
    input.registration.displayPhone,
  );
  ...
  await verifyTurnstile(
    input.registration.turnstileToken,
  ...
  return withTransaction(async (client) => {
    ...
    if (idempotencyRecord) {
      ...
      if (idempotencyRecord.responseReference) {
        ...
        const summary = await getRegistrationSummary(
          idempotencyRecord.responseReference,
          client,
        );
    ...
    const activeCategories = await listActiveCategoriesByEventId(
      event.id,
      client,
    );
    ...
    for (const category of selectedCategories) {
      if (category.participantQuota !== null) {
        const categoryCount = await countActiveCategoryRegistrations(
          category.id,
          client,
        );
    ...
    const emailParticipant = await findActiveParticipantByEmail(
      normalizedEmail,
      client,
    );
    const phoneParticipant = await findActiveParticipantByPhone(
      normalizedPhone,
      client,
    );
    ...
    if (
      (emailParticipant &&
        phoneParticipant &&
        emailParticipant.id !== phoneParticipant.id) ||
    ...
    if (isDatabaseErrorCode(error, '23505')) {
      throw new ApplicationError({
        ...
        safeMessage:
          'Pendaftaran belum dapat diproses karena data sudah digunakan.',
        statusCode: 409,
        cause: error,
      });
    }
...
export async function createParticipantSessionForAccess(input: {
  ...
  const codeHash = hashSensitiveToken(
    input.access.registrationCode.toUpperCase(),
  );
  await assertRateLimit({
    eventId: null,
    ...
    await createParticipantAccessSession(
      {
        ...
        sessionTokenHash: hashSensitiveToken(participantSession.token),
        encryptedRegistrationCode: encryptString(
          input.access.registrationCode.toUpperCase(),
        ),
        csrfTokenHash: hashSensitiveToken(participantSession.csrfToken),
        expiresAt: participantSession.expiresAt,
...
export async function getRegistrationForParticipantSession(
  ...
  const session = await findParticipantSession(
    hashSensitiveToken(sessionToken),
  );
...
function normalizeGlobalParticipantFilters(
  ...
  const pageSize = globalParticipantPageSizes.includes(requestedPageSize)
    ? requestedPageSize
    : 10;
  const status =
    globalParticipantStatuses.find((item) => item === filters.status) ?? null;
...
export async function getGlobalParticipantsForAdmin(input: {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(listResult.totalItems / filters.pageSize),
  );
...
export async function getAdminParticipantDetailForAdmin(input: {
  registrationId: string;
  admin: AuthenticatedAdmin;
}): Promise<AdminParticipantDetail> {
  const summary = await getEventRegistrationForAdmin(input);
  const [submissions, emailDeliveries, bibDocument, validationReviews] =
    await Promise.all([
      listAdminParticipantDetailSubmissions(summary.registration.id),
      listAdminParticipantDetailEmailDeliveries(summary.registration.id),
      getAdminParticipantDetailBibDocument(summary.registration.id),
      listAdminParticipantDetailValidationReviews(summary.registration.id),
    ]);
  ...
  return {
    ...
    bibDocument,
    validationReviews,
    activities: buildAdminParticipantDetailActivities({
  ...
  await withTransaction(async (client) => {
    ...
    if (summary.registration.registrationStatus === 'CANCELLED') {
      throw new ApplicationError({
        ...
        safeMessage:
          'Email tidak dapat dikirim ulang untuk pendaftaran yang dibatalkan.',
        statusCode: 400,
      });
    }
    ...
    if (summary.participant.status !== 'ACTIVE') {
      throw new ApplicationError({
        ...
        safeMessage:
          'Email tidak dapat dikirim ulang untuk peserta yang tidak aktif.',
        statusCode: 400,
      });
    }
...
export async function updateParticipantForAdmin(input: {
  ...
  await withTransaction(async (client) => {
    ...
    await getManageableEvent(summary.event.id, input.admin);
    const participantInput = participantInputFromRegistration(
      input.participant,
    );
    const emailOwner = await findActiveParticipantByEmail(
      participantInput.normalizedEmail,
      client,
    );
    const phoneOwner = await findActiveParticipantByPhone(
      participantInput.normalizedPhone,
      client,
    );
