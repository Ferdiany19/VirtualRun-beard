...
import type {
  ...
  AdminParticipantDetailBibDocument,
  AdminParticipantDetailEmailDelivery,
  AdminParticipantDetailSubmission,
  AdminParticipantDetailValidationReview,
  EmailStatus,
...
type AdminParticipantDetailSubmissionRow = {
...
type AdminParticipantDetailEmailRow = {
  id: string;
  email_type: string;
...
type AdminParticipantDetailBibRow = {
  id: string;
  status: string;
...
type AdminParticipantDetailValidationRow = {
  id: string;
...
function mapGlobalParticipant(
  row: GlobalParticipantRow,
): GlobalParticipantListItem {
  return {
...
function addGlobalParticipantFilters(
  ...
  if (filters.dateTo) {
    ...
    conditions.push(
      `er.registered_at < ($${values.length}::date + interval '1 day')`,
    );
  }
}

function mapAdminDetailSubmission(
  row: AdminParticipantDetailSubmissionRow,
): AdminParticipantDetailSubmission {
  return {
...
function mapAdminDetailEmail(
  row: AdminParticipantDetailEmailRow,
): AdminParticipantDetailEmailDelivery {
  return {
    id: row.id,
...
function mapAdminDetailBib(
  row: AdminParticipantDetailBibRow,
): AdminParticipantDetailBibDocument {
  return {
    id: row.id,
    status: row.status,
...
function mapAdminDetailValidation(
  row: AdminParticipantDetailValidationRow,
): AdminParticipantDetailValidationReview {
  return {
    id: row.id,
    submissionId: row.submission_id,
...
function activityTime(activity: AdminParticipantDetailActivity): number {
  return activity.occurredAt.getTime();
}

export async function getOrCreateBibSettingsForUpdate(
  eventId: string,
  client?: PoolClient,
) {
  await query(
...
export async function listAdminParticipantDetailSubmissions(
  registrationId: string,
  client?: PoolClient,
): Promise<AdminParticipantDetailSubmission[]> {
  const result = await query<AdminParticipantDetailSubmissionRow>(
    `
      SELECT
        ...
        sr.submitted_at,
        sf.id AS file_id,
        sf.original_filename AS file_name,
  ...
  return result.rows.map(mapAdminDetailSubmission);
}

export async function listAdminParticipantDetailEmailDeliveries(
  registrationId: string,
  client?: PoolClient,
): Promise<AdminParticipantDetailEmailDelivery[]> {
  const result = await query<AdminParticipantDetailEmailRow>(
    `
      SELECT
        id,
        email_type,
  ...
  return result.rows.map(mapAdminDetailEmail);
}

export async function getAdminParticipantDetailBibDocument(
  registrationId: string,
  client?: PoolClient,
): Promise<AdminParticipantDetailBibDocument | null> {
  const result = await query<AdminParticipantDetailBibRow>(
    `
      SELECT
        id,
        status,
  ...
  return result.rows[0] ? mapAdminDetailBib(result.rows[0]) : null;
}

export async function listAdminParticipantDetailValidationReviews(
  registrationId: string,
  client?: PoolClient,
): Promise<AdminParticipantDetailValidationReview[]> {
  const result = await query<AdminParticipantDetailValidationRow>(
    `
      SELECT
        vr.id,
        vr.submission_id,
      ...
      JOIN registration_categories rc ON rc.id = vr.registration_category_id
      JOIN event_categories ec ON ec.id = rc.event_category_id
      LEFT JOIN admin_users au ON au.id = vr.reviewer_admin_user_id
      WHERE rc.event_registration_id = $1
  ...
  return result.rows.map(mapAdminDetailValidation);
}

export function buildAdminParticipantDetailActivities(input: {
  ...
  const activities: AdminParticipantDetailActivity[] = [
    {
      id: `registration:${input.registration.id}`,
      action: 'REGISTERED',
      title: 'Pendaftaran baru',
      description: `BIB ${input.registration.bibNumber} terdaftar.`,
      occurredAt: input.registration.registeredAt,
    },
  ];

  for (const email of input.emailDeliveries) {
    activities.push({
      id: `email:${email.id}`,
      action: 'EMAIL',
      title: email.status === 'SENT' ? 'Email terkirim' : 'Email diproses',
      description: email.emailType.replace(/_/g, ' ').toLowerCase(),
      occurredAt: email.sentAt ?? email.createdAt,
    });
  }

  if (input.bibDocument) {
    activities.push({
      id: `bib:${input.bibDocument.id}`,
      action: 'BIB',
      title:
        input.bibDocument.status === 'READY'
          ? 'BIB diterbitkan'
          : 'BIB diproses',
      description:
        input.bibDocument.errorMessage ?? 'Dokumen BIB tercatat oleh sistem.',
      occurredAt: input.bibDocument.generatedAt ?? input.bibDocument.createdAt,
    });
  }
  ...
  for (const submission of input.submissions) {
    if (submission.submittedAt) {
      activities.push({
        id: `submission:${submission.submissionId ?? submission.registrationCategoryId}`,
        action: 'SUBMISSION',
        title:
          submission.submissionStatus === 'APPROVED'
            ? 'Hasil upload diverifikasi'
            : 'Hasil lari diupload',
        description: submission.categoryName,
        occurredAt: submission.approvedAt ?? submission.submittedAt,
      });
    }
  }

  for (const review of input.validationReviews) {
    activities.push({
      id: `validation:${review.id}`,
      action: 'VALIDATION',
      title: 'Review hasil tercatat',
      description: `${review.action.replace(/_/g, ' ').toLowerCase()} - ${review.categoryName}`,
...
export async function listRegistrationsForAdmin(
  ...
  const sortSql =
    ...
    'er.registered_at DESC, er.created_at DESC';
...
export async function listLatestGlobalParticipantsForAdmin(
  ...
  const result = await listGlobalParticipantsForAdmin(
    scope,
    { page: 1, pageSize: 5 },
    client,
  );
